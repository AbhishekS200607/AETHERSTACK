...
// ── Card announcement email ─────────────────────────────────
async function sendCardAnnouncementEmail(name, email, referralCode) {
  const firstName = name.split(' ')[0];
  await transporter.sendMail({
    from: `"Aetherstack" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: `Your Aetherstack ID & Business Card is Ready, ${firstName}!`,
    html: `
      <!DOCTYPE html>
      <html lang="en">
      <head>
              ...
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width:600px;background-color:#ffffff;border-radius:4px;box-shadow:0 10px 30px rgba(0,0,0,0.03);overflow:hidden;">
                ...
                <tr>
                  <td align="left" class="body-font" style="padding:40px 50px 20px;color:#444444;line-height:1.8;font-size:15px;font-weight:300;">
                    ...
                    <p class="premium-font" style="margin:0 0 25px;font-size:20px;color:#111111;line-height:1.4;">
                      Your official <i style="color:#C5A059;">Aetherstack Cards</i> are now ready.
                    </p>

                    <p style="margin-bottom:20px;">We are pleased to inform you that your personalised <strong>Business Card</strong> and <strong>Digital ID Card</strong> are now available for download directly from your marketer profile.</p>

                    <p>These cards carry your name, referral code, and QR link — making it easier than ever to share your referral and represent the Aetherstack brand professionally.</p>

                    <!-- Referral Code Box -->
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin:40px 0;">
                      <tr>
                        <td align="center" style="background-color:#FCFBF7;border:1px solid #EBE4D5;border-radius:4px;padding:35px 20px;">
                          <p class="body-font" style="margin:0 0 12px;font-size:11px;color:#888888;text-transform:uppercase;letter-spacing:2px;">Your Referral Code</p>
                          <p class="premium-font" style="margin:0;font-size:28px;font-weight:500;color:#C5A059;letter-spacing:3px;">${referralCode}</p>
                        </td>
                      </tr>
                    </table>
                    ...
                    <!-- Steps -->
                    <p style="margin-bottom:25px;font-weight:600;color:#111111;">How to download your cards:</p>
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:40px;">
                      ${[
                    ...
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin:10px 0 40px;">
                      <tr>
                        <td align="center">
                          <a href="https://aetherstack.in/marketing-login.html" style="display:inline-block;background-color:#C5A059;color:#ffffff;font-family:'Inter',sans-serif;font-size:14px;font-weight:600;letter-spacing:1px;text-transform:uppercase;text-decoration:none;padding:16px 40px;border-radius:4px;">Go to My Profile</a>
                        </td>
                      </tr>
                    </table>

                    <p>If you have any questions, feel free to reach out to our team at any time.</p>
                    <p style="margin-top:30px;">Keep up the great work — we are proud to have you on the team.</p>
                  </td>
                </tr>
...
// ── GET /api/marketing/admin/claims ────────────────────────────────
router.get('/admin/claims', (req, res, next) => {
  ...
  try {
    const { data, error } = await supabase
      .from('marketers')
      .select('id, name, email, mobile, address, pincode, referral_code, successful_projects, claimed_tier1, claimed_tier2, claimed_tier3, claimed_tier4')
      .or('claimed_tier1.eq.true,claimed_tier2.eq.true,claimed_tier3.eq.true,claimed_tier4.eq.true')
      .order('name');
    if (error) throw error;

    const TIERS = { 1:'Bronze', 2:'Silver', 3:'Gold', 4:'Platinum' };
    const REWARDS = { 1:'Official Aetherstack T-Shirt', 2:'Elite Hoodie + Branded Mug', 3:'Desk Plaque + Tech Kit (Powerbank)', 4:'Premium Backpack + Team Dinner' };
    ...
    data.forEach(m => {
      [1,2,3,4].forEach(t => {
        if (m['claimed_tier' + t]) {
          claims.push({
            marketer_id: m.id, name: m.name, email: m.email,
            mobile: m.mobile, address: m.address, pincode: m.pincode,
            referral_code: m.referral_code, projects: m.successful_projects,
            tier: t, tier_name: TIERS[t], reward: REWARDS[t]
          });
        }
      });
    });
...
// ── POST /api/marketing/admin/dismiss-claim ────────────────────
router.post('/admin/dismiss-claim', (req, res, next) => {
  const secret = req.headers['x-admin-secret'] || req.body.secret;
  if (secret !== process.env.ADMIN_SECRET) return res.status(401).json({ error: 'Unauthorized' });
  next();
}, async (req, res) => {
  try {
    const { marketer_id, tier } = req.body;
    if (!marketer_id || ![1,2,3,4].includes(Number(tier))) return res.status(400).json({ error: 'Invalid params' });
    const col = 'claimed_tier' + tier;
    const { error } = await supabase.from('marketers').update({ [col]: false }).eq('id', marketer_id);
    if (error) throw error;
    return res.json({ success: true });
  } catch (err) {
    return res.status(500).json({ error: err.message });
...
// ── POST /api/marketing/admin/send-card-trial ──────────────────
router.post('/admin/send-card-trial', async (req, res) => {
  const secret = req.headers['x-admin-secret'] || req.body.secret;
  if (secret !== process.env.ADMIN_SECRET) return res.status(401).json({ error: 'Unauthorized' });
  try {
    await sendCardAnnouncementEmail('Abhishek', 'asn69009@gmail.com', 'TEST');
    return res.json({ success: true, message: 'Trial email sent to asn69009@gmail.com' });
  } catch (err) {
    console.error('[send-card-trial]', err.message);
    return res.status(500).json({ error: err.message });
  }
});

// ── POST /api/marketing/admin/broadcast-card ────────────────────
router.post('/admin/broadcast-card', async (req, res) => {
  const secret = req.headers['x-admin-secret'] || req.body.secret;
  if (secret !== process.env.ADMIN_SECRET) return res.status(401).json({ error: 'Unauthorized' });
  try {
    ...
    for (const m of marketers) {
      try {
        await sendCardAnnouncementEmail(m.name, m.email, m.referral_code);
        sent++;
      } catch (e) {
        console.error(`[broadcast-card] failed for ${m.email}:`, e.message);
        failed++;
      }
    }
    return res.json({ success: true, sent, failed, total: marketers.length });
  } catch (err) {
    console.error('[broadcast-card]', err.message);
    return res.status(500).json({ error: err.message });
  }
});
