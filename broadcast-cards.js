...
function buildHtml(name, referralCode) {
  const steps = [
...
async function broadcast() {
  ...
  console.log(`Found ${marketers.length} active marketers. Sending...\n`);
  ...
  for (const m of marketers) {
    try {
      await transporter.sendMail({
        ...
        to: m.email,
        subject: `Your Aetherstack ID & Business Card is Ready, ${m.name.split(' ')[0]}!`,
        html: buildHtml(m.name, m.referral_code)
      });
      console.log(`✅ ${m.name} <${m.email}> — ${m.referral_code}`);
      sent++;
    } catch (e) {
      console.error(`❌ ${m.name} <${m.email}> — ${e.message}`);
      failed++;
    }
  }

  console.log(`\nDone. Sent: ${sent} | Failed: ${failed} | Total: ${marketers.length}`);
}
